...
- 脚本（都在 `Assets/Scripts/`，`partial class Game`）：
  ...
  - `DWHud.cs` — **旧 IMGUI**，现仅剩：虚拟摇杆、队伍小血条、小地图(`GuiMinimap`)、聊天输入、登录界面(`GuiMenu`)。其余已迁 UGUI（`GuiSkillBar/GuiPanel/GuiDeath` 等为死代码，未删）。
  - `DWUguiHud.cs` — **新 UGUI**（Canvas+CanvasScaler 1920×1080）：状态栏/血条经验条、技能栏(6格图标按钮+环形CD+加点)、大攻击键、横幅(战场/BOSS/混战)、信息流、升级/提示/死亡、🎒/🚪、背包/商店/属性滚动面板(品质色图标行)。
...
- ✅ Unity 客户端：连服务器、模型一键接入、碰撞、横屏、音效；**UI 刚从 IMGUI 迁到 UGUI（Phase 1+2）**。
- ⏳ **待办**：Unity UGUI 布局需按真机截图**微调坐标/大小/锚点**（这是盲改，未在编辑器预览）；小地图/队伍 HUD 仍 IMGUI，可迁 UGUI；技能/物品**精美美术图标**（目前是程序色块）需导入图标素材包；用户在做 iOS 真机 Xcode 打包（免费证书 7 天）。
