...
- 脚本（都在 `Assets/Scripts/`，`partial class Game`）：
  ...
  - `Editor/DWSetupWizard.cs` — 菜单「次元大战 → ① 生成资源清单 / ② 一键接入已购模型」：扫描全工程角色，**小丑(LittleWitch1)→世界BOSS**、**骷髅→怪物池(DWMobs)**、其余人物→英雄池(DWHeroes)按次元分配(次元规则:修仙←悟空/魔法←半血/科技←士兵v1/赛博←士兵v2/猎人←牛仔女警长，其余补满)、静态道具→场景池(DWScene 给地图)。排除 URP/HDRP 变体(内置管线会变粉)、残缺部件、**特效/粒子预制体(IsEffectPrefab，防 FX_Waterfall 混进英雄)**，优先 .prefab、折叠颜色变体。①清单还会列出特效预制体路径。**Hovl 特效接入改为独立菜单「③ 接入Hovl特效」(opt-in)——它在内置管线常变粉，所以②不再自动复制**；运行时 `SpawnFx` 用 `FxBroken` 检测 shader 不可用(变粉)就跳过、改用程序化特效。
