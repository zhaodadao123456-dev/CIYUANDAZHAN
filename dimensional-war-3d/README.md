...
- **世界 BOSS 多阶段**：Lv.100 BOSS 有 7 大技能（震地/魔法风暴/弹幕扇射/召唤/陨石/陨石雨/旋转弹幕）。血量 **70% 进入二阶段**（解锁陨石雨与旋转弹幕 bullet-hell + 召唤援军、出招更快），**35% 狂暴**（伤害/速度/频率再升级）；全部地面技能带红圈预警，靠走位躲。
- **次元特技（F 键，各次元独有且更有特色）**：
...
- **装备合成**：背包里 **3 件同品质装备 + 少量金币 → 合成 1 件高一级品质**（自动消耗价值最低的几件），低级散件不再浪费，可一路滚到史诗。
- **装备更难获取**：野怪掉率与高品质权重双双下调，极品装备主要来自世界 BOSS 与大混战。
- **顶级奖励**：世界 BOSS 的 MVP / 高贡献者获得满词条传说装备；**每日五次元大混战的 MVP（击杀最多者）独享【次元至宝】——全武器词条拉满的传说神兵**。
- **可装备成就**：成就不再只是徽章——每个成就附带专属增益（生命/攻击/暴击/吸血/对野怪·玩家·BOSS增伤/金币加成/双抗/全属性等），**每次可装备 1 个**生效；成就门槛已整体调高，越稀有的成就增益越强。
- **满包智能处理**：背包满时新装备不再被白白折现——自动**卖掉背包里品质最低的一件**腾位、保留更好的；可在背包页切换为「🔨 自动强化穿戴装备」，把溢出战利品的金币自动投入强化已穿戴的最低强化件。
