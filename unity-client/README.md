...
`Assets/Resources/DWAudio` 是网页版同款 Juhani Junkala CC0 音频，已随工程提供，**开箱即响**：
攻击/施法、命中、受击、升级、暴击、爆炸（BOSS/范围技）、世界BOSS号角、成就、翻滚都有音效，
登录界面、主世界、战场各有循环 BGM。游戏内按 **M 键静音/开声**（偏好自动记忆）。
逻辑在 `DWAudio.cs`（自包含静态管理器，缺素材时静默不报错）。换音频只需替换 `Resources/DWAudio` 下同名文件。
...
- [x] 断线自动重连、登录信息记忆
- [x] 音乐音效（与网页版同款 CC0，M 键静音）
- [ ] 导入正式角色模型与骨骼动画（向导 ② 一键接入，见上文）
